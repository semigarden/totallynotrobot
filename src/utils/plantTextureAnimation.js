...
export const updateAtlasPlantTexture = (mesh, plantId, globalProgress) => {
    const renderData = mesh.userData?.plantRenderData?.get(plantId);
    const context = mesh.userData?.atlasContext;
    const texture = mesh.userData?.atlasTexture;

    if (!renderData || !context || !texture) return false;
