...
const toPlantSeed = (garden, item) => ({
    id: `${garden.id}:${item.id}`,
    text: item.text || item.title,
    title: item.title,
    blogTitle: item.blogTitle || item.title,
    link: item.link,
    gardenId: garden.id,
    gardenName: garden.name,
    pubDate: item.pubDate || null,
    at: item.pubDate ? Date.parse(item.pubDate) || Date.now() : Date.now(),
});

const anchorScore = (plant, anchor) => {
    const gapDays = daysBetween(plant.at, anchor.at);
    const timeScore = 1.2 / (gapDays + 1);
    const sameAuthor = plant.gardenId === anchor.gardenId;
    const authorScore = sameAuthor ? 0.18 + 0.35 / (gapDays / 180 + 1) : 0;

    return timeScore + authorScore;
};
...
const buildUnifiedDateLayout = (plants) => {
    ...
    yearGroups.forEach(([year, yearPlants]) => {
        ...
        yearPlants.forEach((plant, index) => {
            ...
            const gapDays = daysBetween(plant.at, anchor.at);
            const authorPull = plant.gardenId === anchor.gardenId ? 0.72 : 1;
            const gapScale =
                (1 + Math.min(gapDays / 45, 1.6) * 0.18) * authorPull;
...
export const layoutNewPlantByDate = (plant, existingPlants = []) => {
    ...
    const gapDays = daysBetween(plant.at, anchor.at);
    const authorPull = plant.gardenId === anchor.gardenId ? 0.72 : 1;
    const gapScale = (1 + Math.min(gapDays / 45, 1.6) * 0.18) * authorPull;
...
export const gardensToForestPlants = (gardens, feedResults) => {
    const seeds = gardens.flatMap((garden) => {
        const feed = feedResults[garden.id];
        const items = feed?.items ?? garden.fallbackItems ?? [];
        return items.map((item) => toPlantSeed(garden, item));
    });

    const layoutById = buildUnifiedDateLayout(seeds);

    return seeds.map((plant) => {
        const position = layoutById.get(plant.id) ?? { x: 0, z: 0 };

        return withChunkFields({
            ...plant,
            x: position.x,
            z: position.z,
            minSpacing: position.minSpacing,
        });
    });
};

export const computeGardenCentroids = (gardens, plants) =>
    gardens.map((garden) => {
        const gardenPlants = plants.filter((plant) => plant.gardenId === garden.id);
        ...
        const totals = gardenPlants.reduce(
            (acc, plant) => ({
                x: acc.x + plant.x,
                z: acc.z + plant.z,
            }),
        ...
        return {
            ...garden,
            centerX: totals.x / gardenPlants.length,
            centerZ: totals.z / gardenPlants.length,
        };
    });
